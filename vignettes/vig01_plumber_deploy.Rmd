...
One important thing to note about the above example is that it is **insecure**. The service is exposed over HTTP, and there is no authentication layer: anyone on the Internet can contact the service and potentially interact with it. Depending on your use case, this may not be a problem; however; it's highly recommended that you should provide at least some level of authentication, as well as restricting the service to HTTPS only. You can also deploy the AKS cluster as a private cluster, which will make it visible only to hosts on its own subnet. 
