...
We'll use Traefik to handle authentication details. A key benefit of this is that it frees our model container to focus purely on its assigned task of returning predicted values. For example, if we want to switch to a different kind of model, or a different server package (say, using RestRserve instead of Plumber), we can do so without having to replicate any code for user authentication.
...
Once the deployment is complete, we can obtain predicted values from it. Notice that we use the default HTTPS port for the request (port 443); while port 8000 is internally exposed by the service, this is visible only within the cluster; it is connected to the external port 443 by Traefik.
